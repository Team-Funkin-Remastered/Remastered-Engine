...
{
	var mods:Array<DLCMenuState.ModMetadata> = [];
	...
	override function create()
	{
		...
		{
			FlxG.sound.play(Paths.sound('scrollMenu'), 0.6);
			FlxG.switchState(new DLCMenuState());
		});
		...
		while (i < modsList.length)
		{
			...
			var newMod:DLCMenuState.ModMetadata = new DLCMenuState.ModMetadata(path + '/' + values[0], true);
			mods.push(newMod);
	...
	function moveMod(change:Int, skipResetCheck:Bool = false)
	{
		if(mods.length > 1)
		{
			...
			else
			{
				...
				var lastMod:DLCMenuState.ModMetadata = mods[curSelected];
